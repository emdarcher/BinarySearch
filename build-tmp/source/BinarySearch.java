...
public int binarySearch(int catNumToFind){
    int low=0; int high=store.length-1;
    while(low<=high){
...
public void setup(){
    ...
    for (int i = 0; i < tests.length; i++){
        if(linearSearch(tests[i]) != -1){
            System.out.println("Catalog #"+tests[i]+" has "
                +linearSearch(tests[i]) + " in stock");
        } else {
    ...
    for (int i = 0; i < tests.length; i++){
        if(binarySearch(tests[i],0,store.length - 1) != -1){
            System.out.println("Catalog #"+tests[i]+" has "
                +binarySearch(tests[i],0,store.length - 1) + " in stock");
        } else {
            System.out.println("Catalog #"+tests[i]+" not found");
        }
    }
